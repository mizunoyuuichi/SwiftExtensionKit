...
public enum ProText: View {
    case head    (_ style: TextStyle, _ text: String)
    case subHead (_ style: TextStyle, _ text: String)
    case label   (_ style: TextStyle, _ text: String)
    case body    (_ style: TextStyle, _ text: String)
    case caption (_ style: TextStyle, _ text: String)
    case footnote(_ style: TextStyle, _ text: String)

    public var body: some View {
        switch self {
        case .head(let style, let text):
            Text(text)
                .font(.system(size: 18, weight: .bold))
                .foregroundStyle(Color.k90)
                .withTextStyle(style)
        case .subHead(let style, let text):
            Text(text)
        ...
        case .label(let style, let text):
            Text(text)
                ...
                .foregroundStyle(Color.k80)
                .withTextStyle(style)
        case .body(let style, let text):
            Text(text)
                .font(.system(size: 16, weight: .regular))
                .foregroundStyle(Color.k70)
                .withTextStyle(style)
        case .caption(let style, let text):
            Text(text)
                .font(.system(size: 14, weight: .light))
                .foregroundStyle(Color.k50)
                .withTextStyle(style)
        case .footnote(let style, let text):
            Text(text)
                .font(.system(size: 14, weight: .light))
                .foregroundStyle(Color.k50)
                .withTextStyle(style)
...
#Preview {
    ...
    VStack(spacing: 4) {
        ProText.head(sampleText)
        ProText.head(.airily, sampleText)
        ProText.head(.relaxedly, sampleText)
        ProText.head(.formally, sampleText)
        ProText.head(.tightly, sampleText)

        ProText.subHead(sampleText)
